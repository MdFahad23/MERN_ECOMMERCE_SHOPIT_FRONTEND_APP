...
const ShippingAddress = () => {
  ...
  const { loading, message, errors, address, redirect } = useSelector(
  ...
  const handelSubmit = (e) => {
    e.preventDefault();
    dispatch(AddShippingAddress(userInfo().jwt, values));
  };
  ...
  useEffect(() => {
    ...
    dispatch(clearError());
  }, [alert, message, errors, dispatch, address]);
      ...
      <div className="container">
        {loading ? (
          ...
          <div className="py-[80px] md:w-[800px] m-auto">
            {redirect && navigate("/shipping-details")}
            <form
